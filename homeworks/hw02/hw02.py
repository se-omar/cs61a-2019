...
def pingpong(n):
    ...
    # is_reverse = False
    # while count < n:
    #     if count % 7 == 0 or num_sevens(count) > 0:
...
def pingpong_helper(count, curr_num, is_reverse, n):
    if count == n:
        return curr_num
    if count % 7 == 0 or num_sevens(count) > 0:
        return pingpong_helper(count + 1, curr_num + 1 if is_reverse else curr_num - 1, not is_reverse, n)
    return pingpong_helper(count + 1, curr_num - 1 if is_reverse else curr_num + 1, is_reverse, n)
